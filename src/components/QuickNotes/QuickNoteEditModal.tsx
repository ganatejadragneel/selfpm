...
interface QuickNoteEditModalProps {
  note: QuickNote;
  onClose: () => void;
}

export const QuickNoteEditModal: React.FC<QuickNoteEditModalProps> = ({ note, onClose }) => {
  const theme = useThemeColors();
  const { notes, updateNote } = useQuickNotesStore();
  ...
  const handleSave = async () => {
    ...
    const finalTags = tagInput.trim()
      ...
      : tags;
    try {
      await updateNote(note.id, title.trim(), content.trim(), finalTags);
      onClose();
    } catch (e) {
